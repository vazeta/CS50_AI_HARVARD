...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        """
        # 1) Marca a jogada como feita
        self.moves_made.add(cell)

        # 2) Marca a célula como segura
        self.mark_safe(cell)

        # 3) Constrói sentença com os vizinhos da célula
        neighbors = set()
        i, j = cell
        ...
        for s1 in self.knowledge:
            for s2 in self.knowledge:
                ...
                if s1.cells.issubset(s2.cells):
                    ...
                    if new_sentence not in self.knowledge and new_sentence not in new_sentences and len(diff_cells) > 0:
